...
public class ShadowZoneMovement : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        refractionVariaton = ShadowZoneGraph.angleVariationAtDepths[(int)depth];
    ...
    void Update()
    {
        ...
        if (started)
        {
            ...
            if (Mathf.Abs(newTheta) <= 2f && !needToGoUp)
                needToGoUp = true;
        ...
        if (Mathf.Abs(transform.position.y) >= 1999 || transform.position.y >= 0)
        {
            ...
            {
                theta = -firtsThetaValue-30; 
                started = false; // ? mettere il rimbalzo?
            }
    ...
    {
        return (ShadowZoneGraph.speedAtDepths[(int)y + 1] - ShadowZoneGraph.speedAtDepths[(int)y]) / (y + 1 - y);
    }
    ...
    {
        return ShadowZoneGraph.speedAtDepths[(int)y];
    }
