...
public class NegativeGradient : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        for (int i = 0; i < 11; i++)
        {
            ...
            targets[i].gameObject.name = "NegativeSVPtarget " + i;
        }
    }
    ...
    public IEnumerator Run(int i, float gradi)
    {
       Debug.Log("target: "+ (i+1) + "| gradi: " + gradi + "| radianti: " + (Mathf.Deg2Rad*gradi));
        ...
        {
            range += speed;
            //siccome è negative gradient
            float negativeGradient = Mathf.Abs(targets[i].transform.position.y)/5;
            targets[i].transform.position = new Vector3((range * Mathf.Cos(Mathf.Deg2Rad * gradi)), ((-1)* (range * Mathf.Sin(Mathf.Deg2Rad * gradi)))-negativeGradient, 5);
