...
public class ConstantGradientSVP : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (started)
        {
            ...
            if(Mathf.Abs(newTheta) <= 0.5f){
                theta = theta + 1f;
            }


            transform.position = new Vector3(xPos + speed * Mathf.Cos(Mathf.Deg2Rad * (theta)), (-depth + speed * Mathf.Sin(Mathf.Deg2Rad * (newTheta))), 0);
            lineRenderer.positionCount++;
            lineRenderer.SetPosition(test, transform.position);
            test++;
        }
        ...
        if (Mathf.Abs(transform.position.y) >= 1999 || transform.position.y >= 0)
        {
            bounce++;
            theta = -theta;
            if (Mathf.Abs(transform.position.y) >= 1999) //Aggiunto ora
                needToGoUp = false; //Aggiunto ora
        }
    }
